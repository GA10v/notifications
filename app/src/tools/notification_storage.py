...
from models.content import Content, ContentType, ReviewContent
...
class NotificationStorage(AbsNotificationStorage):
    ...
    async def get_review_content(self, content_id: UUID) -> Optional[Content]:
        result = await self.session.execute(
            select(Content).where(Content.content_id == content_id, Content.content_type == ContentType.review_like),
        )
    ...
    async def registrate_welcome_notification(self, user_id: UUID):
        notification = Notification(
            content_id=user_id,
            content_type=ContentType.new_user,
            last_update=datetime.datetime.utcnow(),
        )
        self.session.add(notification)
        await self.session.commit()
